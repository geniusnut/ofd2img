...
class Image(MultiMedia):
    def __init__(self, node, _zf):
        ...
        # Normalize location for matching (remove leading slash if present)
        location_variants = [self.location]
        if self.location.startswith("/"):
            location_variants.append(self.location[1:])
        if not self.location.startswith("/"):
            location_variants.append("/" + self.location)

        def find_in_zip(zf, name_variants):
            for loc in zf.namelist():
        ...
        if suffix == "jb2":
            jb2_path = find_in_zip(_zf, location_variants)
            if not jb2_path:
                raise FileNotFoundError(
                    f"JBIG2 image resource not found in OFD: {self.location}"
                )
            ...
            png_path = find_in_zip(_zf, location_variants)
            if not png_path:
                raise FileNotFoundError(
                    f"PNG image resource not found in OFD: {self.location}"
                )
            tmp_folder = os.path.basename(_zf.filename).replace(".ofd", "")
...
def res_add_drawparams(node, _zf):
    # First pass: add all DrawParams without considering Relative
    for draw_param in node.children:
        DrawParams[draw_param.attr["ID"]] = DrawParam(draw_param)

    # Second pass: apply Relative inheritance
    for draw_param in node.children:
        param_id = draw_param.attr["ID"]
        if "Relative" in draw_param.attr:
            relative_id = draw_param.attr["Relative"]
            if relative_id in DrawParams:
                # Get the base DrawParam
                base_param = DrawParams[relative_id]
                current_param = DrawParams[param_id]

                # Inherit properties from base if not explicitly set in current
                if current_param.line_width == 0.25 and base_param.line_width != 0.25:
                    current_param.line_width = base_param.line_width
                if current_param.stroke_color == [
                    0,
                    0,
                    0,
                ] and base_param.stroke_color != [0, 0, 0]:
                    current_param.stroke_color = base_param.stroke_color
                if current_param.fill_color == [0, 0, 0] and base_param.fill_color != [
                    0,
                    0,
                    0,
                ]:
                    current_param.fill_color = base_param.fill_color
